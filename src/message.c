...
int message_send(const Message msg, Connection_Info* conn_info){
    ...
    &header,sizeof(Message_Header),MSG_NOSIGNAL | MSG_MORE))
    MESSAGE_SOCKET_ERROR(send(conn_info->client_info.socket,
    buff,final_size,MSG_NOSIGNAL))
    if(ntohl(header.message_type) == Text_Message_Encrypted)
        free(buff);
}
int message_receive(Message* msg,Connection_Info* conn_info){
    Message_Header header;
    MESSAGE_SOCKET_ERROR(recv(conn_info->client_info.socket,&header,sizeof(Message_Header),MSG_NOSIGNAL));
    header.message_type = ntohl(header.message_type);
    header.size = ntohl(header.size);
    if(header.size > 0){
        char* buff = malloc(header.size);
        MESSAGE_SOCKET_ERROR(recv(conn_info->client_info.socket,buff,header.size,MSG_NOSIGNAL));
        if(header.message_type == Text_Message_Encrypted){
